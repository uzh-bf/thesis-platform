...
```bash
infisical run --env=prod --projectId "$INFISICAL_PROJECT_ID" -- pnpm exec prisma migrate deploy
```
...
EXPECTED_POSTGRES_HOST="$EXPECTED_POSTGRES_HOST" \
EXPECTED_POSTGRES_DATABASE="$EXPECTED_POSTGRES_DATABASE" \
pnpm prod:db:migrate:execute -- --wipe-target
```
